...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        // Mengambil daftar file formulir dari database
    ...
    case 'PUT':
        ...
        if (isset($inputData['id']) && isset($inputData['file_name'])) {
            ...
            $sql = "UPDATE formulir_deteksi_tumbuh_kembang SET pengumpulan_dokumen = '$newFileName' WHERE id = $id";
            if ($conn->query($sql) === TRUE) {
                echo json_encode([
                    'status' => 'success',
    ...
    case 'DELETE':
        ...
        if (isset($inputData['id'])) {
            ...
            $sql = "SELECT pengumpulan_dokumen FROM formulir_deteksi_tumbuh_kembang WHERE id = $id";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                $row = $result->fetch_assoc();
                $fileToDelete = '../uploads/formulir_deteksi_dan_tumbuh_kembang/' . $row['pengumpulan_dokumen'];
                ...
                $sqlDelete = "DELETE FROM formulir_deteksi_tumbuh_kembang WHERE id = $id";
